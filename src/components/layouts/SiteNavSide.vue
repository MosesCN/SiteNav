<template>
  <el-dialog
    v-model="MENU_DIALOG.show"
    :title="MENU_DIALOG.title"
    width="18%"
    ...
    <el-form>
      <el-form-item :label="MENU_DIALOG.iconLabel">
        <el-select
          ...
          style="width: 80%"
          filterable
          v-model="MENU_DIALOG.icon"
        >
          <el-option
            v-for="item in ep_icons"
            :key="item"
            :label="item"
            :value="item"
          />
      ...
      <el-form-item
        :label="MENU_DIALOG.addSubMenuLabel"
        v-if="MENU_DIALOG.subMenuId == undefined"
      >
        <el-switch v-model="MENU_DIALOG.addSubMenu" />
      </el-form-item>
      <el-form-item
        :label="MENU_DIALOG.subIconLabel"
        v-if="MENU_DIALOG.subMenuId == undefined && MENU_DIALOG.addSubMenu"
      >
        <el-select
          size="large"
          style="width: 80%"
          filterable
          v-model="MENU_DIALOG.subIcon"
        >
          <el-option
            v-for="item in ep_icons"
            :key="item"
      ...
      <el-form-item
        :label="MENU_DIALOG.subNameLabel"
        v-if="MENU_DIALOG.subMenuId == undefined && MENU_DIALOG.addSubMenu"
      >
        <el-input
          v-model="MENU_DIALOG.subName"
          maxlength="6"
          style="width: 80%"
          show-word-limit
  ...
  <div id="SiteNavSide">
    ...
    >
      ...
      >
        <template #title>
          <DropdownAction
            @add="addMenu(menu, undefined)"
            @delete="delMenu(menu, undefined)"
        ...
        <el-menu-item :index="subMenus.id" v-for="subMenus in menu.subMenus">
          <DropdownAction
            @add="addMenu(menu, subMenus)"
            @delete="delMenu(menu, subMenus)"
...
const confirmMenuDialog = () => {
  ...
  const foundMenu = menus.value.find(
    (m: any) => m.id == MENU_DIALOG.value.menuId
  );
  if (MENU_DIALOG.value.type == ACTION_TYPE_EDIT) {
    ...
    const addingIdx = eidtingSubMenu
      ? foundMenu.subMenus.findIndex(
          (m: any) => m.id == MENU_DIALOG.value.subMenuId
        )
